...
export const submitDOI = async (formData: TRaftContext): Promise<IResponseData<string>> => {
  ...
  try {
    ...
    const multipartFormData = createDoiFormData({ metaData: convertedJSON })
    ...
    // POST without suffix = create new DOI. Backend returns 303 redirect on success.
    ...
    // DOI created successfully - now upload RAFT.json with metadata (both must succeed)
    const raftDataWithMeta: TRaftContext = {
      ...formData,
    ...
    const uploadResult = await uploadFile(identifier, raftDataWithMeta, accessToken)
