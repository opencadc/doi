...
public class PostAction extends DoiAction {
    ...
    public static void assignIdentifier(Object ce, String identifier) {
        try {
            ...
            throw new RuntimeException(
                "Identifier class is missing the value field",
                fex
            );
        } catch (IllegalAccessException bug) {
            throw new RuntimeException(
                "No access to Identifier value field",
                bug
            );
        }
    }
    ...
    private Resource merge(Resource sourceResource, Resource targetResource) {
        ...
        targetResource
            .getPublicationYear()
            .setValue(sourceResource.getPublicationYear().getValue());
    ...
    private void updateResource(Resource resourceFromUser) throws Exception {
        ...
        Resource resourceFromVos = vospaceDoiClient.getResource(
            doiSuffix,
            nodeName
        );
        Resource mergedResource = merge(resourceFromUser, resourceFromVos);
        VOSURI docVOSURI = getVOSURI(
            String.format("%s/%s", doiSuffix, getDoiFilename(doiSuffix))
        );
    ...
    private void updateJournalRef(String journalRefFromUser) throws Exception {
        ...
        ContainerNode doiContainerNode = vospaceDoiClient.getContainerNode(
            doiSuffix
        );
        VOSURI vosuri = getVOSURI(doiSuffix);
        String journalRefFromVOSpace = doiContainerNode.getPropertyValue(
            DOI_VOS_JOURNAL_PROP
        );
        if (journalRefFromVOSpace == null) {
            if (!journalRefFromUser.isEmpty()) {
                // journal reference does not exist, add it
                NodeProperty journalRef = new NodeProperty(
                    DOI_VOS_JOURNAL_PROP,
                ...
                vospaceDoiClient
                    .getVOSpaceClient()
                    .setNode(vosuri, doiContainerNode);
            }
        } else {
            if (!journalRefFromUser.isEmpty()) {
                // journal reference already exists, update it
                doiContainerNode
                    .getProperty(DOI_VOS_JOURNAL_PROP)
                    .setValue(journalRefFromUser);
            } else {
                // delete existing journal reference
                NodeProperty nodeProperty = doiContainerNode.getProperty(
                    DOI_VOS_JOURNAL_PROP
                );
                if (nodeProperty != null) {
                    ...
                    doiContainerNode
                        .getProperties()
                        .add(new NodeProperty(DOI_VOS_JOURNAL_PROP));
                }
            }
            vospaceDoiClient
                .getVOSpaceClient()
    ...
    private String getCredentials() {
        return String.format(
            ...
            config.getFirstPropertyValue(
                DoiInitAction.DATACITE_MDS_USERNAME_KEY
            ),
            config.getFirstPropertyValue(
                DoiInitAction.DATACITE_MDS_PASSWORD_KEY
            )
        );
    }
    ...
    private void registerDOI(
        ...
        // post to DataCite
        HttpPost postToDataCite = new HttpPost(
            postURL,
            content,
            contentType,
            redirect
        );
        postToDataCite.setRequestProperty(
            "Authorization",
            "Basic " + Base64.encodeString(getCredentials())
        );
        postToDataCite.run();
        ...
        processResponse(
            postToDataCite.getThrowable(),
            postToDataCite.getResponseCode(),
            postToDataCite.getResponseBody(),
            msg
        );
    }
    ...
    private void makeDOIFindable(ContainerNode doiContainerNode)
        ...
        String landingPageUrl = config.getFirstPropertyValue(
            DoiInitAction.LANDING_URL_KEY
        );
    ...
    private String getDOIContent() throws Exception {
        Resource resource = vospaceDoiClient.getResource(
            doiSuffix,
            getDoiFilename(doiSuffix)
        );
    ...
    private void register(ContainerNode doiContainerNode) throws Exception {
        ...
        try {
            // update status
            doiContainerNode
                .getProperty(DOI_VOS_STATUS_PROP)
                .setValue(Status.REGISTERING.getValue());
            vospaceDoiClient
                .getVOSpaceClient()
                .setNode(doiURI, doiContainerNode);

            // register DOI to DataCite
            String dataCiteUrl = config.getFirstPropertyValue(
                DoiInitAction.DATACITE_MDS_URL_KEY
            );
            URL registerURL = new URL(
                String.format(
                    "%s/metadata/%s/%s",
                    dataCiteUrl,
                    cadcPrefix,
            ...
            doiContainerNode
                .getProperty(DOI_VOS_STATUS_PROP)
                .setValue(Status.MINTED.getValue());
    ...
    private void lockData(ContainerNode doiContainerNode) throws Exception {
        ...
        try {
            // update status
            ContainerNode dataContainerNode = vospaceDoiClient.getContainerNode(
                doiDataPath
            );
            doiContainerNode
                .getProperty(DOI_VOS_STATUS_PROP)
                .setValue(Status.LOCKING_DATA.getValue());
            vospaceDoiClient
                .getVOSpaceClient()
                .setNode(containerVOSURI, doiContainerNode);
            ...
            vospaceDoiClient
                .getVOSpaceClient()
                .setNode(dataVOSURI, dataContainerNode);

            // get the job URL
            RecursiveSetNode recSetNode = vospaceDoiClient
                .getVOSpaceClient()
                .createRecursiveSetNode(dataVOSURI, dataContainerNode);
            ...
            log.debug(
                "invoked async call to recursively set the properties in the data directory " +
                doiDataPath
            );

            // save job URL
            NodeProperty jobURLProp = new NodeProperty(
                DOI_VOS_JOB_URL_PROP,
                jobURL.toExternalForm()
            );
            ...
            String jobURLString = doiContainerNode.getPropertyValue(
                DOI_VOS_JOB_URL_PROP
            );
            if (jobURLString != null) {
                doiContainerNode
                    .getProperties()
                    .remove(new NodeProperty(DOI_VOS_JOB_URL_PROP));
            }
    ...
    private void performDoiAction() throws Exception {
        if (doiAction.equals(DoiAction.MINT_ACTION)) {
            ...
            Status mintingStatus = Status.toValue(
                doiContainerNode.getPropertyValue(DOI_VOS_STATUS_PROP)
            );
            switch (mintingStatus) {
                ...
                case LOCKING_DATA:
                    ...
                    log.debug(
                        "doi " + doiSuffix + " status: " + Status.LOCKING_DATA
                    );
                    break;
                case LOCKED_DATA:
                ...
                case REGISTERING:
                    ...
                    log.debug(
                        "doi " + doiSuffix + " status: " + Status.REGISTERING
                    );
                    break;
                case MINTED:
                ...
                case COMPLETED:
                    ...
                    log.debug(
                        "doi " + doiSuffix + " status: " + Status.COMPLETED
                    );
                    break;
                default:
                // do nothing
            ...
            throw new UnsupportedOperationException(
                "DOI action not implemented: " + doiAction
            );
        }
    }
    ...
    private void updateDOI() throws Exception {
        ...
        Resource resourceFromUser = (Resource) syncInput.getContent(
            DoiInlineContentHandler.CONTENT_KEY
        );
    ...
    private void verifyImmutableFields(Resource r1, Resource r2) {
        if (
            !r1
                .getNamespace()
                .getPrefix()
                .equals(r2.getNamespace().getPrefix()) ||
            !r1.getNamespace().getURI().equals(r2.getNamespace().getURI())
        ) {
            String msg = String.format(
                "namespace update is not allowed, expected: %s, actual: %s",
                r2.getNamespace(),
                r1.getNamespace()
            );
            ...
            String msg = String.format(
                "software error, publisher is different, expected: %s, actual: %s",
                r2.getPublisher(),
                r1.getPublisher()
            );
            throw new IllegalArgumentException(msg);
    ...
    private void verifyString(String s1, String s2, String field) {
        ...
        if (!s1.equals(s2)) {
            String msg = String.format(
                "%s update is not allowed, expected: %s, actual: %s",
                field,
                s2,
                s1
            );
    ...
    private void verifyNull(Object o1, Object o2, String field) {
        if (o1 == null && o2 != null) {
            String msg = String.format(
                "%s update is not allowed, expected: %s, actual: null",
                field,
                o2
            );
            throw new IllegalArgumentException(msg);
            ...
            String msg = String.format(
                "%s update is not allowed, expected: null, actual: %s",
                field,
                o1
            );
            throw new IllegalArgumentException(msg);
    ...
    private void verifyIdentifier(Identifier i1, Identifier i2) {
        if (!i1.equals(i2)) {
            String msg = String.format(
                "identifier update is not allowed, expected: %s, actual: %s",
                i2,
                i1
            );
            throw new IllegalArgumentException(msg);
    ...
    private void verifyResourceType(ResourceType rt1, ResourceType rt2) {
        ...
        if (rt1.getResourceTypeGeneral() != rt2.getResourceTypeGeneral()) {
            String msg = String.format(
                "resourceType update is not allowed, expected: %s, actual: %s",
                rt2.getResourceTypeGeneral().getValue(),
    ...
    private void createDOI() throws Exception {
        // Get the submitted form data, if it exists
        Resource resource = (Resource) syncInput.getContent(
            DoiInlineContentHandler.CONTENT_KEY
        );
        if (resource == null) {
            throw new IllegalArgumentException("No content");
        ...
        boolean randomName = Boolean.parseBoolean(
            config.getFirstPropertyValue(DoiInitAction.TEST_RANDOM_NAME_KEY)
        );
        ...
        if (randomName) {
            ...
            nextDoiSuffix = getNextDOISuffix(
                vospaceDoiClient.getDoiBaseVOSURI()
            );
        ...
        GroupURI guri;
        String configuredGroupUri = config.getFirstPropertyValue(
            DoiInitAction.TEST_GROUP_URI_KEY
        );
        ...
        vospaceDoiClient
            .getVOSpaceClient()
            .createNode(doiDocVOSURI, doiDocNode);
        ...
        vospaceDoiClient
            .getVOSpaceClient()
            .createNode(dataVOSURI, newDataFolder);
    ...
    private GroupURI createDoiGroup(String groupName) throws Exception {
        ...
        String gmsResourceID = config.getFirstPropertyValue(
            DoiInitAction.GMS_RESOURCE_ID_KEY
        );
    ...
    private ContainerNode createDOIDirectory(GroupURI guri, String folderName)
        ...
        NodeProperty doiRequestor = new NodeProperty(
            DOI_VOS_REQUESTER_PROP,
            this.callersNumericId.toString()
        );
        properties.add(doiRequestor);

        NodeProperty doiStatus = new NodeProperty(
            DOI_VOS_STATUS_PROP,
            Status.DRAFT.getValue()
        );
    ...
    private void uploadDOIDocument(Resource resource, VOSURI docVOSUIRI)
        ...
        ClientTransfer clientTransfer = vospaceDoiClient
            .getVOSpaceClient()
        ...
        if (clientTransfer.getThrowable() != null) {
            ...
            if (clientTransfer.getThrowable() != null) {
                ...
                throw new RuntimeException(
                    (clientTransfer.getThrowable().getMessage())
                );
            }
        }
    }
    ...
    private String getNextDOISuffix(VOSURI baseDoiURI) throws Exception {
        ContainerNode baseNode = (ContainerNode) vospaceDoiClient
            .getVOSpaceClient()
            .getNode(baseDoiURI.getPath());
